...
- (void)setDictionary:(NSDictionary *)dictionary;
{
    ...
    for (CPObjectProperty * property in [self.class propertyItems])
    {
                ...
                if ([propertyType hasPrefix:@"T@\""])
                {
                    ...
                    else if (class_isClass(objClass, NSArray.class))
                    {
                        ...
                        for (id v in value)
                        {
                            ...
                            if (class_isClass(objClass, CPObject.class))
                            {
                    ...
                    } else if (class_isClass(objClass, CPObject.class)) {
                        // 数据模型对象是属性，自动生成对象
...
- (NSDictionary *)dictionaryFormInfoWithNull:(BOOL)withNull
{
    ...
    for (CPObjectProperty * property in [self.class propertyItems])
    {
        ...
        } else if ([value isKindOfClass:CPObject.class]) {
            [dic setObject:[value dictionaryFormInfo] forKey:key];
        } else if ([value isKindOfClass:NSArray.class]) {
            NSMutableArray * arr = [NSMutableArray array];
            for (id obj in value) {
                if ([obj isKindOfClass:CPObject.class]) {
