...
{
    NSString *string = [[self toStringWithFormat:@"yyyy/MM/dd HH:mm:ss"] stringByReplacingOccurrencesOfString:@" " withString:@"T"];
    return string;
}
...
-(NSString*)toChatString {
    ...
    if (selftime > now) {  // 将来的时间
        str = [self toStringWithFormat:[NSString stringWithFormat:@"yyyy年%@月%@日 %@:mm", m, d, h]];
        ...
        if (distime < 5 * 60) {
            ...
            if (selftime > yesterday) {
                ...
                str = [self toStringWithFormat:[NSString stringWithFormat:@"%@月%@日 %@:mm", m, d, h]];
            } else {
                str = [self toStringWithFormat:[NSString stringWithFormat:@"yyyy年%@月%@日 %@:mm", m, d, h]];
            }
        }
...
-(NSString *)getDayString {
    return [[NSString stringWithFormat:@"%d", (int)[self getDay]] stringByAppendingString:@"日"];
}
